    ...
    public sealed class ApplicationDbContextFactory : IDesignTimeDbContextFactory<ApplicationDbContext>
    {
        public ApplicationDbContext CreateDbContext(string[] args)
        {
            var connectionString = Environment.GetEnvironmentVariable("PRODUCT_MANAGER_CONNECTION_STRING")
                ...
                ?? "Server=(localdb)\\MSSQLLocalDB;Database=ProductManager;Trusted_Connection=True;TrustServerCertificate=True;";
        ...
        private static string? TryReadWebUiConnectionString()
        {
            ...
            {
                var appSettingsPath = Path.Combine(current.FullName, "ProductManager.WebUI", "appsettings.json");
